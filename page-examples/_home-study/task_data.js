...
_set_req=function(){
    ...
    sql+=",task as (select ID,UID,PUID,S3,Site=participant.Site,Information,participant_sql,DateTime,Author,RowNum=row_number() over (order by ID DESC) from [FORM-"+_db_pid+"-@S1] join participant on PUID=ParticipantUID)";
...
_set_req_export=function(i1,i2){
    ...
    sql+=",records as (select ID,ParticipantUID,Site,Information,Participant=participant_sql,DateTime,Author,RowNum=row_number() over (order by ID DESC) from participant left join task on PUID=ParticipantUID)";
